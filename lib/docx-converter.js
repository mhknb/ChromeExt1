...
<Properties xmlns="http://schemas.openxmlformats.org/officeDocument/2006/extended-properties">
  ...
  /**
   * Create proper ZIP using fflate (if available) or manual implementation
   */
  async createProperZip(files) {
    // Import fflate dynamically
    try {
      const { strToU8, zipSync } = await import('https://esm.sh/fflate@0.8.1');
      ...
      return new Blob([zipped], { type: this.mimeType });
    } catch (error) {
      console.error('fflate not available, DOCX may fail:', error);
      // Return error - we need proper ZIP
      throw new Error('DOCX oluşturmak için gerekli kütüphane yüklenemedi');
    }
  }
}
