...
class PdfConverterBundled {
  ...
  async exportToPdf(markdown) {
    try {
      ...
      // Try to add Noto Emoji font for emoji support (may not work with all jsPDF versions)
      ...
      try {
        ...
        console.log('[PDF] NotoEmoji font loaded successfully');
      } catch (error) {
        console.warn('[PDF] Failed to load NotoEmoji font:', error.message);
        console.log('[PDF] Emoji will be converted to text equivalents');
      }
      ...
      const processInlineText = (text, keepBold = false) => {
        ...
        // Convert emojis based on whether emoji font is available
        if (emojiFontAvailable) {
          // Use Unicode equivalents (will be rendered with NotoEmoji font)
          ...
          text = text.replace(/✅/g, '+'); // Check mark → plus
          text = text.replace(/❌/g, 'X'); // Cross mark → X
          text = text.replace(/✔️/g, '+'); // Heavy check mark → plus
          text = text.replace(/❎/g, 'X'); // Cross mark button → X
          text = text.replace(/☑️/g, '+'); // Ballot box with check → plus
          text = text.replace(/✓/g, '+'); // Check mark → plus
          text = text.replace(/✗/g, 'X'); // Ballot X → X
          text = text.replace(/🔴/g, 'O'); // Red circle → O
          text = text.replace(/🟢/g, 'O'); // Green circle → O
          text = text.replace(/⭕/g, 'O'); // Hollow red circle → O
        }
        ...
        switch (token.type) {
          ...
          case 'table':
            ...
            for (let col = 0; col < table.header.length; col++) {
              ...
              const height = lines.length * 4.5 + (cellPadding * 2);
            ...
            for (let col = 0; col < table.header.length; col++) {
              ...
              // Set colors for this specific cell BEFORE drawing
              ...
              // Use NotoEmoji font if available and text contains emoji/symbols
              const useEmojiFont = emojiFontAvailable && hasEmojiOrSymbols(headerText);
              if (useEmojiFont) {
            ...
            for (let rowIdx = 0; rowIdx < table.rows.length; rowIdx++) {
              ...
              const rowFillColor = (rowIdx % 2 === 0) ? [255, 255, 255] : [250, 250, 250];
              ...
              for (let col = 0; col < row.length; col++) {
                ...
                // Set colors for this specific cell BEFORE drawing
                pdf.setFillColor(rowFillColor[0], rowFillColor[1], rowFillColor[2]); // White or light gray
                pdf.setDrawColor(180, 180, 180); // Gray borders
                pdf.setTextColor(0, 0, 0); // Black text
                ...
                // Use NotoEmoji font if available and text contains emoji/symbols
                const useEmojiFont = emojiFontAvailable && hasEmojiOrSymbols(cellText);
                if (useEmojiFont) {
                  pdf.setFont('NotoEmoji', 'normal');
                }
